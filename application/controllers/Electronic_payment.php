<?php
defined('BASEPATH') OR exit('No direct script access allowed');

class Electronic_payment extends MY_Controller {

  public function __construct() {
    parent::__construct();
    $this->load->model('E_payment_model', 'e_payment');
    $this->load->model('Js_model', 'js');

    // Check in application/core/MY_Controller the value of $this->region and $this->company
    if ($_SESSION['company'] == 8) {
      $this->region  = $this->mdi_region;
      $this->company = $this->mdi;
    }
  }

  public function index() {
    $this->access(1);
    $this->header_data('title', 'E-payment List');
    $this->header_data('nav', 'e_payment');
    $this->header_data('dir', './');

    $param = new Stdclass();
    $param->date_from = $this->input->post('date_from');
    $param->date_to = $this->input->post('date_to');
    $param->region = $this->input->post('region');
    $param->status = $this->input->post('status');
    $param->reference = $this->input->post('reference');

    $data['table']   = $this->e_payment->get_list($param);
    $data['status']  = $this->e_payment->status;
    $data['region']  = $this->region;
    $data['company'] = $this->company;
    $data['acctg'] = in_array($_SESSION['dept_name'], array('Accounting', 'Treasury'));
    $this->template('e_payment/list', $data);
  }

  public function extract() {
    $this->access(1);
    $this->header_data('title', 'E-payment Extract');
    $this->header_data('nav', 'e_payment');
    $this->header_data('dir', './../');

    $data['company'] = ($this->session->company === 'MDI') ? array(8 => 'MDI') : array(1 => 'MNC', 6 => 'MTI', 3 => 'HPTI');
    $this->template('e_payment/extract_form', $data);
  }

  public function csv() {
    $param = new Stdclass();
    $param->region = $_SESSION['region_id'];
    $param->company = $this->input->post('company');
    $param->date_from = $this->input->post('date_from');
    $param->date_to = $this->input->post('date_to');

    $data['result'] = $this->e_payment->extract_to_csv($param);
    $data['param'] = $param;
    $this->load->view('e_payment/extract_csv', $data);
  }

  public function view($epid) {
    $payment = $this->e_payment->load_payment($epid);
    $this->access(1);
    $this->header_data('title', $payment->reference);
    $this->header_data('nav', 'e_payment');
    $this->header_data('dir', './../../');

    $payment->region  = $this->region[$payment->region];
    $payment->company = $this->company[$payment->company];

    $data['payment'] = $payment;
    $this->template('e_payment/view', $data);
  }

  public function add()
  {
    $this->access(1);
    $this->header_data('title', 'Add New Batch');
    $this->header_data('nav', 'e_payment');
    $this->header_data('dir', './../');

    $save = $this->input->post('save');
    if (!empty($save)) {
      $this->form_validation->set_rules('company', 'Company', 'required');
      $this->form_validation->set_rules('reference', 'Reference #', 'required');
      $this->form_validation->set_rules('ref_date', 'Date', 'required');
      $this->form_validation->set_rules('amount', 'Amount', 'required|is_numeric|non_zero');

      if ($this->form_validation->run()
        // && ($screenshot = $this->e_payment->upload_screenshot())
        && ($batch = $this->e_payment->upload_batch())) {
        $payment = new Stdclass();
        $payment->region = $_SESSION['region_id'];
        $payment->company = $this->input->post('company');
        $payment->reference = $this->input->post('reference');
        $payment->ref_date = $this->input->post('ref_date');
        $payment->amount = $this->input->post('amount');
        // $payment->screenshot = $screenshot;
        $this->e_payment->save_payment($payment, $batch);
      }
    }

    $data['region'] = $this->region[$_SESSION['region_id']];
    $data['company'] = array(0 => '- Please select a company -', 1 => 'MNC', 3 => 'HPTI', 6 => 'MTI');
    $this->template('e_payment/add', $data);
  }

  public function edit($epid) {
    $save = $this->input->post('save');
    if (!empty($save)) {
      $this->form_validation->set_rules('company', 'Company', 'required');
      $this->form_validation->set_rules('reference', 'Reference #', 'required');
      $this->form_validation->set_rules('ref_date', 'Date', 'required');
      $this->form_validation->set_rules('amount', 'Amount', 'required|is_numeric|non_zero');

      if ($this->form_validation->run()) {
        $new_payment = new Stdclass();
        $new_payment->epid = $epid;
        $new_payment->region = $_SESSION['region_id'];
        $new_payment->company = $this->input->post('company');
        $new_payment->reference = $this->input->post('reference');
        $new_payment->ref_date = $this->input->post('ref_date');
        $new_payment->amount = $this->input->post('amount');

        // if (isset($_FILES['screenshot'])) $new_payment->screenshot = $this->e_payment->upload_screenshot();
        $remove = $this->input->post('remove');
        $engine_no = $this->input->post('engine_no');

        $this->e_payment->update_payment($new_payment, $remove, $engine_no);
      }
    }

    $payment = $this->e_payment->load_payment($epid);

    $this->access(1);
    $this->header_data('title', 'Edit '.$payment->reference);
    $this->header_data('nav', 'e_payment');
    $this->header_data('dir', './../../');

    $data['payment'] = $payment;
    $data['epid'] = $epid;
    $data['region'] = (isset($_SESSION['region_id'])) ? $this->region[$_SESSION['region_id']] : '';
    $data['company'] = array(0 => '- Please select a company -', 1 => 'MNC', 6 => 'MTI', 3 => 'HPTI');
    $this->template('e_payment/edit', $data);
  }

  public function pending()
  {
    $this->access(1);
    $this->header_data('title', 'Pending E-Payment');
    $this->header_data('nav', 'e_payment');
    $this->header_data('dir', './../');

    $save = $this->input->post('save');
    if (!empty($save)) {
      $doc_no = $this->input->post('doc_no');
      if (!empty($doc_no)) {
        foreach ($doc_no as $epid => $val) {
          if (!empty($val)) {
            $payment = new Stdclass();
            $payment->epid = $epid;
            $payment->doc_no = $val;
            $payment->doc_date = date('Y-m-d H:i:s');
            $this->e_payment->update_payment_status($payment, 2);
          }
        }
        $_SESSION['messages'][] = 'Records updated successfully.';
      }
      else {
        $_SESSION['warning'][] = 'Nothing to save.';
      }
      redirect($_SERVER['HTTP_REFERER']);
    }

    $data['table']   = $this->e_payment->get_list_by_status(1);
    $data['region']  = $this->region;
    $data['company'] = $this->company;
    $this->template('e_payment/pending', $data);
  }

  public function processing()
  {
    $this->access(1);
    $this->header_data('title', 'Processing E-Payment');
    $this->header_data('nav', 'e_payment');
    $this->header_data('dir', './../');

    $save = $this->input->post('save');
    if (!empty($save)) {
      $dm_no = $this->input->post('dm_no');
      if (!empty($dm_no)) {
        foreach ($dm_no as $epid => $val) {
          if (!empty($val)) {
            $payment = new Stdclass();
            $payment->epid = $epid;
            $payment->dm_no = $val;
            $payment->dm_date = date('Y-m-d H:i:s');
            $this->e_payment->update_payment_status($payment, 3);
          }
        }
        $_SESSION['messages'][] = 'Records updated successfully.';
      }
      else {
        $_SESSION['warning'][] = 'Nothing to save.';
      }
        redirect($_SERVER['HTTP_REFERER']);
    }

    $data['table'] = $this->e_payment->get_list_by_status(2);
    $data['region'] = $this->region;
    $data['company'] = $this->company;
    $this->template('e_payment/processing', $data);
  }

  public function for_deposit() {
    $this->access(1);
    $this->header_data('title', 'For Deposit E-Payment');
    $this->header_data('nav', 'e_payment');
    $this->header_data('dir', './../');
    $checkboxid    = 'check-all-for-deposit';
    $checkboxclass = 'input-checkbox';

    $save = $this->input->post('save');
    if (!empty($save)) {
      $epids = $this->input->post('epid');
      if (!empty($epids)) {
        $ctr = 0;
        foreach ($epids as $epid) {
          $payment = new Stdclass();
          $payment->epid = $epid;
          $payment->deposit_date = date('Y-m-d H:i:s');
          $this->e_payment->deposit_payment($payment);
          $ctr++;
        }
        if ($ctr) $_SESSION['messages'][] = 'Records updated successfully.';
      } else {
        $_SESSION['warning'][] = 'Nothing to save.';
      }
      redirect($_SERVER['HTTP_REFERER']);
    }

    $data['table'] = $this->e_payment->get_list_by_status(3);
    $data['region']  = $this->region;
    $data['company'] = $this->company;
    $data['id_select_all']  = $checkboxid;
    $data['class_checkbox'] = $checkboxclass;
    $data['javascript']  = $this->js->jquery_checkall($checkboxid, $checkboxclass);
    $this->template('e_payment/for_deposit', $data);
  }

  public function liquidation()
  {
    $this->access(1);
    $this->header_data('title', 'Liquidation E-Payment');
    $this->header_data('nav', 'e_payment');
    $this->header_data('dir', './../');

    $save = $this->input->post('save');
    if (!empty($save)) {
      $liquidated = $this->input->post('liquidated');
      if (!empty($liquidated)) {
        foreach ($liquidated as $epid) {
          $payment = new Stdclass();
          $payment->epid = $epid;
          $payment->close_date = date('Y-m-d H:i:s');
          $this->e_payment->update_payment_status($payment, 5);
        }
        $_SESSION['messages'][] = 'Records updated successfully.';
      }
      else {
        $_SESSION['warning'][] = 'Nothing to save.';
      }
      redirect($_SERVER['HTTP_REFERER']);
    }

    $data['table'] = $this->e_payment->get_liquidation_list();
    $data['region']  = $this->region;
    $data['company'] = $this->company;
    $this->template('e_payment/liquidation', $data);
  }

  public function print_batch($epid)
  {
    $payment = $this->e_payment->load_batch_sales($epid);
    $payment->region = $this->region[$payment->region];
    $payment->company = $this->company[$payment->company];
    $data['payment'] = $payment;
    $this->load->view('e_payment/print_batch', $data);
  }

  public function add_amt()
  {
    $addtl_amt = $this->input->post('add_amt');
    list($epid) = array_keys($addtl_amt);
    $this->db->trans_start();
    $this->db->query("
      UPDATE tbl_electronic_payment
      SET addtl_amt = addtl_amt + {$addtl_amt[$epid]},
          status = 1
      WHERE epid = {$epid}
    ");
    $this->db->trans_complete();

    if ($this->db->trans_status()) {
      $_SESSION['messages'][] =  'Amount updated successfully!';
    } else {
      $_SESSION['warning'][] = 'Something went wrong.';
    }

    redirect($_SERVER['HTTP_REFERER']);
  }

}
