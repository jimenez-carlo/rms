  ...
  public function __construct() { 
	...
	public function index()
	{
		...
		switch ($_SESSION['position'])
		{
			...
			*/
			...
			default: $this->template('home_view');
		}
	}
	...
	public function unprocessed()
	{
		...
		$unprocessed = $rms->query("
			...
			CASE WHEN pnp_status=0 OR pnp_status IS NULL
					THEN 'PNP'
				 WHEN (ar_no IS NULL OR ar_no='N/A') AND (regn_status <> 'Self Registration' OR regn_status <> 'Free Registration')
					THEN 'AR'
				 WHEN si_date_received IS NULL AND rrt_class <> 'NCR'
					THEN 'SI'
				 WHEN insurance_date IS NULL
					THEN 'INSURANCE'
				 ELSE 'PENDING'
		         END AS status
			FROM customer_tbl
	...
	private function ccn($data = array())
	{
		...
		if ($bnew_total > 0)
		{
			...
			if ($bnew_rejected > 0)
			{
				$bnew_data .= '{label: "Rejected", value: '.$bnew_rejected.' },';
				$bnew_color .= '"#A11717",';
			}
			if ($bnew_pending > 0)
			{
		...
		// orcr
		$result = $this->db->query("SELECT count(*), ts.status FROM tbl_transmittal_sales ts
	...
	private function rrt($data = array())
	{
		...
		$region = ($_SESSION['position'] == 107) ? '1 = 1' : 'ph_region = '.$_SESSION['region'];
		...
		$result = $this->db->query("select count(*) as count,
			...
			from tbl_rerfo r
			inner join portal_global_db.tbl_branches on r.branch = bid
			inner join tbl_sales s
				on s.branch = r.branch
				and left(sales_type, 9) = type
	...
	private function rrt_spvsr($data = array())
	{
		...
		$result = $this->db->query("SELECT count(*) as count,
			...
			from tbl_sales
			inner join portal_global_db.tbl_branches on branch = bid
			where ".$region."
			group by 2")->result_object();
	...
	private function acct($data = array())
	{
		...
		$result = $this->db->query("select count(*) as count,
			case when status = 0 then 'unprocessed'
				when status = 1 and (select count(*) from tbl_topsheet_sales
					where batch = 0 and topsheet = tid) > 0 then 'incomplete'
				when status = 1 and (select count(*) from tbl_topsheet_sales
					where batch = 0 and topsheet = tid) = 0 then 'sap_upload'
		...
		/* batch
		$result = $this->db->query("select count(*) as count,
			case when doc_no is null then 'sap'
				when valid_doc_no is null then 'offset'
				when valid_doc_no <> '0' and voucher_no is null then 'voucher'
				else 'done'
			end as status
			from tbl_batch
			group by 2")->result_object();
		...
		$b_data = '';
		$b_color = '';
		$b_total = $data['b_sap'] + $data['b_offset'] + $data['b_voucher'] + $data['b_done'];

		if ($b_total > 0)
		{
			$b_sap = round((($data['b_sap'] / $b_total) * 100), 2);
			$b_offset = round((($data['b_offset'] / $b_total) * 100), 2);
			$b_voucher = round((($data['b_voucher'] / $b_total) * 100), 2);
			$b_done = round((($data['b_done'] / $b_total) * 100), 2);

			if ($b_sap > 0)
			{
				$b_data .= '{label: "For SAP Upload", value: '.$b_sap.' },';
				$b_color .= '"#A11717",';
			}
			if ($b_offset > 0)
			{
				$b_data .= '{label: "For CA Offset", value: '.$b_offset.' },';
				$b_color .= '"#BDBB19",';
			}
			if ($b_voucher > 0)
			{
				$b_data .= '{label: "For Voucher", value: '.$b_voucher.' },';
				$b_color .= '"#BDBB19",';
			}
			if ($b_done > 0)
			{
				$b_data .= '{label: "Done", value: '.$b_done.' },';
				$b_color .= '"#3DA117",';
			}
		}
		else
		{
			$b_sap = $b_offset = $b_voucher = $b_done = 0;
