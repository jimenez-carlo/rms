...
class Fund_transfer_model extends CI_Model {
  ...
  public function get_repo_for_deposit() {
    return $this->db
      ->select("
        rb.repo_batch_id, rb.reference, rb.doc_no,
        DATE_FORMAT(rb.date_created, '%Y-%m-%d') AS date_created,
        (rb.amount + rb.bank_amount) AS amount,
        r.region
      ")
      ->join('tbl_region r', 'r.rid = rb.region_id','left')
      ->get_where('tbl_repo_batch rb', 'rb.status = "FOR DEPOSIT"')
      ->result_object();
  }
